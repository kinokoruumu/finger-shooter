...
export const DraggableBar = ({
	...
	const cursorForDrag =
		dragging === "move" ? "grabbing" : dragging ? "ew-resize" : "grab";

	const actualWidth = Math.max(barWidth, 24);
	const hasResize = !!onDrag;

	return (
		// biome-ignore lint/a11y/useKeyWithClickEvents: ドラッグ操作専用のバー
		<div
							...
							{delayRatio != null && sR > dR && sR < 1 && onDrag && (
								<div
									...
									className={cn(
										...
										dragging === "resize-spawn" ? "bg-white/30" : "bg-white/10",
									)}
									style={{ left: `calc(${sR * 100}% - 6px)` }}
									onPointerDown={(e) => handlePointerDown(e, "resize-spawn")}
								/>
							)}
							{/* 表示残り（やや薄い） */}
							{sR < 1 && (
			...
			{hasResize && (
				<div
					...
					className={cn(
						...
						dragging === "resize-left" ? "bg-white/30" : "bg-white/10",
					)}
			...
			{hasResize && (
				<div
					...
					className={cn(
						...
						dragging === "resize-right" ? "bg-white/30" : "bg-white/10",
					)}
